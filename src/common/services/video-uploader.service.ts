...
export class VideoUploaderService {
  private s3: S3;
  private readonly logger = new Logger(VideoUploaderService.name);
  ...
  async uploadFileToS3(file: Express.Multer.File): Promise<any> {
    ...
    if (!bucketName) {
      this.logger.error('AWS_S3_BUCKET is not defined in environment variables');
      throw new InternalServerErrorException('S3 bucket configuration error');
    }
    ...
    managedUpload.on('httpUploadProgress', (progress) => {
      ...
      const percent = Math.round((uploaded / contentLength) * 100);
      this.logger.log(`Uploading ${file.originalname}: ${percent}%`);
    });
