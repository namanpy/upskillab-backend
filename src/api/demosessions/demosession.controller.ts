...
export class DemoSessionController {
  ...
  @ApiResponse({ status: 201, description: 'Create a new demo session' })
  @Post('')
  ...
  @ApiResponse({ status: 200, description: 'Get a single demo session by ID' })
  @Get(':id')
  async getDemoSessionById(@Param('id') id: string) {
    return await this.demoSessionLogicService.getDemoSessionById(id);
  }

  @ApiResponse({ status: 200, description: 'Update a demo session by ID' })
  @Put(':id')
