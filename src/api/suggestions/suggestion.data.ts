...
export class SuggestionDataService {
  constructor(@InjectModel(Suggestion.name) private suggestionModel: Model<SuggestionDocument>) {}

  async createSuggestion(createSuggestionDto: CreateSuggestionDTO & { teacherId: string; content: string }): Promise<SuggestionDocument> {
  ...
  async getSuggestionsByBatch(batchIds: string[], isApproved: boolean): Promise<SuggestionDocument[]> {
  ...
  async getSuggestionsByTeacher(teacherId: string): Promise<SuggestionDocument[]> {
    return this.suggestionModel
      .find({ teacherId })
      .populate(['batchId', 'teacherId'])
      .lean()
  ...
  async updateSuggestion(id: string, updateSuggestionDto: Partial<UpdateSuggestionDTO>): Promise<SuggestionDocument | null> {
    return this.suggestionModel
  ...
  async getSuggestionById(id: string): Promise<SuggestionDocument | null> {
    return this.suggestionModel.findById(id).populate(['batchId', 'teacherId']).lean().exec();
  }
}